...
import { useGetMapPresalesOrg } from "@/features/presales/api/use-get-map-presales-org";
import Loader from "@/components/loader";
import { useGetRegions } from "@/features/regions/api/use-get-regions";
import { ResponseType } from "@/features/presales/api/use-get-map-presales-org";
...
function Map({ height = "600px", type, developerId }: MapProps) {
  ...
  const { data: presales } = useGetMapPresalesOrg({
...
const PresalesMapOrg = ({ height, type, developerId }: MapProps) => {
  ...
  return <Map height={height} type={type} developerId={developerId} />;
};

export default PresalesMapOrg;
