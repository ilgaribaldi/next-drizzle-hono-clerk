...
export const Navbar = () => {
  ...
  return (
    <motion.div ref={ref} className="w-full fixed top-0 inset-x-0 z-[51]">
      <DesktopNav
        visible={visible}
        navItems={navItems}
        isAdmin={isAdmin}
      ...
      />
      <MobileNav visible={visible} navItems={navItems} isAdmin={isAdmin} />
    </motion.div>
  );
};
      ...
      <motion.div className="lg:flex flex-row flex-1 absolute inset-0 hidden items-center justify-center space-x-2 lg:space-x-2 text-sm text-zinc-600 font-medium hover:text-zinc-800 transition duration-200">
        {navItems.map((navItem: any, idx: number) => (
          <Link
            ...
            className="text-neutral-500 dark:text-neutral-300 relative px-4 py-2"
            key={`link=${idx}`}
            href={navItem.link}
          >
            {hovered === idx && (
                ...
                if (isAdmin) {
                  return (
                    <Button
                      ...
                      href="/admin"
                      variant="secondary"
                      className="hidden md:block text-neutral-500"
                    >
                      Dashboard
                    </Button>
                  );
